...
class gaussianEnergy:
    def __init__(self, params):
        self.params = params
        self.means = jnp.array([mode[0] for mode in params.modes])
        self.sigmas = jnp.array([mode[1] for mode in params.modes])
        self.weights = jnp.array([mode[2] for mode in params.modes])
        self.weights /= self.weights.sum()
        self.configuration_shape = (2,)

    def f(self, x):
        x = jnp.reshape(x, (1, 2))
        Si = 0.5 * ((x - self.means) ** 2).sum(-1) / (self.sigmas**2)
    ...
    def diffusionfT(self, x, T, base_sigma):
        ...
        Si = 0.5 * ((x - means) ** 2).sum(-1) / (sigmas**2)
